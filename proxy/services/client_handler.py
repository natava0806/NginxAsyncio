...
from proxy.core.config import HTTP_HEADER_DELIMITER, ProxySettings
...
class ClientConnectionHandler:
    ...
    async def handle(self) -> None:
        ...
        while keep_alive and not self._client_writer.is_closing():
            try:
                # ИСПРАВЛЕНО: Используем self._settings вместо глобального settings
                keep_alive = await asyncio.wait_for(
                    self._process_single_request(),
                    timeout=self._settings.timeouts.total
                )
            except TimeoutError:
    ...
    async def _process_single_request(self) -> bool:
        ...
        while HTTP_HEADER_DELIMITER not in header_data:
            # ИСПРАВЛЕНО: Используем self._settings
            chunk = await TimeoutPolicy.run_with_timeout(
                self._client_reader.read(4096),
                self._settings.timeouts.read, "reading headers from client"
            )
            if not chunk:
        ...
        try:
            ...
            # ИСПРАВЛЕНО: Используем self._settings
            await TimeoutPolicy.run_with_timeout(up_writer.drain(), self._settings.timeouts.write,
            ...
            if client_keep_alive and not self._upstream_should_close and not up_writer.is_closing():
    ...
    async def _stream_client_to_upstream(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter,
        ...
        if content_length > 0:
            ...
            while bytes_sent < content_length:
                ...
                data = await TimeoutPolicy.run_with_timeout(reader.read(to_read), self._settings.timeouts.read,
                                                            "reading client body")
                if not data:
                    break
                writer.write(data)
                await TimeoutPolicy.run_with_timeout(writer.drain(), self._settings.timeouts.write,
                                                     "draining upstream buffer")
                bytes_sent += len(data)
        elif is_chunked:
            while True:
                # ИСПРАВЛЕНО: Используем self._settings
                chunk = await TimeoutPolicy.run_with_timeout(reader.read(self._buffer_size),
                                                             self._settings.timeouts.read,
    ...
    async def _stream_upstream_to_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        while HTTP_HEADER_DELIMITER not in header_data:
            chunk = await TimeoutPolicy.run_with_timeout(
                reader.read(4096), self._settings.timeouts.read, "reading upstream headers"
            )
            if not chunk:
                return
            header_data += chunk
        ...
        # Отправка клиенту заголовков ответа
        ...
        if up_headers.get('connection', '').lower() == 'close':
            self._upstream_should_close = True
    ...
    async def _stream_fixed_content(self, reader: asyncio.StreamReader, body_start: bytes, content_length: int) -> None:
        ...
        while bytes_sent < content_length:
            ...
            data = await TimeoutPolicy.run_with_timeout(
                reader.read(to_read), self._settings.timeouts.read, "reading upstream body"
            )
            if not data:
                break
            ...
            await TimeoutPolicy.run_with_timeout(
                self._client_writer.drain(), self._settings.timeouts.write, "draining client body response buffer"
            )
            bytes_sent += len(data)
